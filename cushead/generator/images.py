...
@overload
def get_resized_image(*, image: IcoImagePlugin.IcoImageFile, width: int,
                      height: int) -> IcoImagePlugin.IcoImageFile:
    ...


@overload
def get_resized_image(*, image: PngImagePlugin.PngImageFile, width: int,
                      height: int) -> PngImagePlugin.PngImageFile:
    ...
...
@overload
def get_opaque_image(*, image: PngImagePlugin.PngImageFile,
                     background_color: str) -> PngImagePlugin.PngImageFile:
    ...
...
def get_image_bytes(*, image: Optional[
        Union[IcoImagePlugin.IcoImageFile, PngImagePlugin.PngImageFile]]
                    ) -> bytes:
    """
...
def generate_images(*, config: generator_config.Config) -> List[files.File]:
    ...
    if config.get("favicon_png"):
        ...
        resized_image = get_resized_image(image=config["favicon_png"],
                                          width=image.width,
                                          height=image.height)
        if config.get("background_color"):
            parsed_image = get_opaque_image(image=resized_image,
                                            background_color=str(
                                                config["background_color"]))
        else:
            parsed_image = resized_image
        images.append(
            files.File(data=get_image_bytes(image=parsed_image),
                       path=image.path))
