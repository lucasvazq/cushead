...
def get_resized_image(
    *, image: IcoImagePlugin.IcoImageFile, width: int, height: int
...
@overload
def get_resized_image(
    *, image: PngImagePlugin.PngImageFile, width: int, height: int
...
def get_opaque_image(
    *, image: PngImagePlugin.PngImageFile, background_color: str
) -> PngImagePlugin.PngImageFile:
    ...
...
def get_image_bytes(
    *, image: Optional[Union[IcoImagePlugin.IcoImageFile, PngImagePlugin.PngImageFile]]
) -> bytes:
    """
...
def generate_images(*, config: generator_config.Config) -> List[files.File]:
    ...
    if config.get("favicon_png"):
        images_data = (
            ...
            ImageData(
                path=config["output_folder_path"] / "static" / "favicon-16x16.png",
                width=16,
                height=16,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "favicon-32x32.png",
                width=32,
                height=32,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "favicon-96x96.png",
                width=96,
                height=96,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "favicon-192x192.png",
                width=192,
                height=192,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "favicon-194x194.png",
                width=194,
                height=194,
            ),
            # Apple icon.
            ...
            ImageData(
                path=config["output_folder_path"]
                / "static"
                / "browserconfig-144x144.png",
                width=144,
                height=144,
            ),
            # manifest.
            ImageData(
                path=config["output_folder_path"] / "static" / "manifest-192x192.png",
                width=192,
                height=192,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "manifest-512x512.png",
                width=512,
                height=512,
            ),
            # Apple startup image.
            ...
            ImageData(
                path=config["output_folder_path"]
                / "static"
                / "apple-touch-startup-image-1024x1024.png",
                width=1024,
                height=1024,
            ),
            ImageData(
        ...
        # Yandex.
        image = ImageData(
            path=config["output_folder_path"] / "static" / "yandex.png",
            width=120,
            height=120,
        )
        ...
        images.append(
            files.File(data=get_image_bytes(image=parsed_image), path=image.path)
        )
    ...
    if config.get("preview_png"):
        images_data = (
            # Open Graph.
            ImageData(
                path=config["output_folder_path"] / "static" / "preview-600x600.png",
                width=600,
                height=600,
            ),
            ImageData(
                path=config["output_folder_path"] / "static" / "preview-1080x1080.png",
                width=1080,
                height=1080,
            ),
            # Twitter Cards.
