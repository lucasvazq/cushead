...
class TestConfig(base_tests.BaseTests):
    ...
    def compare_output(self, *, template_folder_path: pathlib.Path) -> None:
        ...
        self.assertEqual(
            [str(file.relative_to(self.output_folder))
             for file in self.output_folder.rglob("*")],
            [str(file.relative_to(template_folder))
        ...
        for generated_file in self.output_folder.rglob("*"):
            template_file = template_folder / \
                str(generated_file.relative_to(self.output_folder))
