...
class IndexGenerator(src.base.generator.images.Images):
    ...
    def index_body(self):
        ...
        return f'\n{src.helpers.INDENTATION}'.join([
            f"<body>",
            f"{src.helpers.INDENTATION}{body_content}",
            f"</body>",
        ]).replace("'", '"')
...
class ComplementaryFilesGenerator:
    ...
    def complementary_browserconfig(self) -> typing.Dict[str, str]:
        ...
        return {
            ...
            "destination_file_path": os.path.join(self.config["static_folder_path"], "browserconfig.xml"),
        }
    ...
    def complementary_humans(self) -> typing.Dict[str, str]:
        ...
        content.extend([
            "/* SITE */",
            f"{src.helpers.INDENTATION}Last update: *",
        ])
        ...
        return {
            ...
            "destination_file_path": os.path.join(self.config["static_folder_path"], "humans.txt"),
        }
    ...
    def complementary_manifest(self) -> typing.Dict[str, str]:
        ...
        if self.icons_config["manifest"]:
            ...
            content["icons"] = [
                {
                    'src': f"{self.config['static_url']}/{manifest_config.output_file_name}-{size}x{size}",
                    'sizes': f'{size}x{size}',
                    'type': 'image/png',
        ...
        return {
            "content": json.dumps(content, indent=src.helpers.INDENTATION),
            "destination_file_path": os.path.join(self.config["static_folder_path"], "manifest.json"),
        }
    ...
    def complementary_opensearch(self) -> typing.Dict[str, str]:
        ...
        return {
            ...
            "destination_file_path": os.path.join(self.config["static_folder_path"], "opensearch.xml"),
        }
    ...
    def complementary_robots(self) -> typing.Dict[str, str]:
        ...
        if 'domain' in self.config:
            content.extend([
                f"",
                f"Sitemap: https://{self.config['domain']}/sitemap.xml",
            ])

        return {
            "content": '\n'.join(content),
            "destination_file_path": os.path.join(self.config["output_folder_path"], "robots.txt"),
        }

    def complementary_security(self) -> typing.Union[typing.Dict[str, str], None]:
        ...
        return {
            "content": '\n'.join(content).replace("'", '"'),
            "destination_file_path": os.path.join(self.config["output_folder_path"], ".well-known/humans.txt"),
        }
    ...
    def complementary_service_worker(self):
        ...
        return {
            "content": content,
            "destination_file_path": os.path.join(self.config["output_folder_path"], "sw.js"),
        }

    def complementary_sitemap(self) -> typing.Union[typing.Dict[str, str], None]:
        """sitemap.xml content
        ...
        return {
            "content": ("<?xml version='1.0' encoding='utf-8'?>\n" + f'\n{src.helpers.INDENTATION}'.join(content) + "\n</urlset>").replace("'", '"'),
            "destination_file_path": os.path.join(self.config["output_folder_path"], "sitemap.xml"),
        }
...
class ImagesGenerator:
    ...
    def get_icons_creation_config(
        ...
        return [element for group in icons_creation_config for element in group]
