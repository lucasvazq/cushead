...
class Images:
    ...
    def icons_head_creator(
        ...
        # tag name, example:
        # link
        tag_element_list.append(f"{brand_config.tag_name} " if getattr(
            brand_config, "tag_name", "") else "")

        # color, example:
        # ???
        tag_element_list.append(
            f"color='{brand_config.attribute_color}' " if getattr(
                brand_config, "attribute_color", "") else "")

        # content, example:
        # ???
        tag_element_list.append(
            f"content='{brand_config.attribute_content}' " if getattr(
                brand_config, "attribute_content", "") else "")

        # property, example:
        # ???
        tag_element_list.append(
            f"property='{brand_config.attribute_property}' " if getattr(
                brand_config, "attribute_property", "") else "")

        # name, example:
        # name="msapplication-TileImage"
        tag_element_list.append(
            f"name='{brand_config.attribute_name}' " if getattr(
                brand_config, "attribute_name", "") else "")

        # rel, example:
        # rel="icon"
        tag_element_list.append(
            f"rel='{brand_config.attribute_rel}' " if getattr(
                brand_config, "attribute_rel", "") else "")
        ...
        tag_element_list.append(f"title='{title}' " if getattr(
            brand_config, "attribute_special_title", False) else "")
